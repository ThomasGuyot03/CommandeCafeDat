...
printBL(order) {
  ...
  // ✅ Ouvre une nouvelle fenêtre pour le BL
  const printWindow = window.open('', '_blank', 'width=900,height=1000');

  // ✅ Contenu du BL injecté dans la nouvelle fenêtre
          ...
          <div class="content">
            ...
            <div class="footer">
              <div class="footer-info">
                <div><strong>Articles</strong><div>${order.products.length}</div></div>
                <div><strong>Quantité Totale</strong><div>${order.products.reduce((sum, p) => sum + p.quantity, 0)}</div></div>
                <div><strong>Statut</strong><div style="color: #2ecc71;">✓ En cours</div></div>
              </div>
  ...
  printWindow.onload = () => {
    printWindow.focus();
    printWindow.print();
    printWindow.close(); // ferme la fenêtre automatiquement
  };
}
  },
  mounted() {
